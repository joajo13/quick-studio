...
- Given a pushed `v*` tag, when the workflow completes, then windows-x64, linux-x64, and linux-arm64 binaries plus `SHA256SUMS` are attached to the release.
- Given the platform list, when macOS is added in a later phase, then it is adding rows to one shared list — not editing the matrix, the shim map, and the packaging script independently.
...
- `.github/workflows/release.yml` — replace the two-entry `include` matrix with three native-runner entries (`runs-on` per leg: `windows-latest`, `ubuntu-latest`, `ubuntu-24.04-arm`); drop the `--target` flags; drop `max-parallel: 1`; each leg uploads its binary as a **workflow artifact** rather than straight to the release. Keep the entries shaped so darwin rows can be appended later without touching anything else.
...
- `README.md` — the Install section's binary list currently names only Linux and Windows x64; extend it to the three published assets, state plainly that macOS is a later phase, and document `sha256sum -c SHA256SUMS` verification.
