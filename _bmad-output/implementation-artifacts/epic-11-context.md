...
- **Release matrix**: each platform binary is compiled on its own native runner, never cross-compiled, because `@napi-rs/keyring` is a native NAPI addon whose binding is resolved at build time. Each leg gates on the SHIPPED binary itself: `release.yml` runs `./quick-studio-<os>-<arch>` with `QS_SELFCHECK=keychain`, a hidden env-gated branch in `bin/quick-studio.ts` that performs the keychain round-trip and exits before the Core boots, and fails the release unless the run prints `selfcheck: OK`. (Superseded DW-89: the gate originally compiled and ran a SECOND artifact from `scripts/keyring-native-check.ts`, which proved the leg's toolchain could embed `@napi-rs/keyring` but never touched the published binary. That script is still the entrypoint `keyring-spike.yml` compiles, and both paths now share one implementation in `src/core/keychain-self-check.ts`.) Every release attaches a `SHA256SUMS` file.
