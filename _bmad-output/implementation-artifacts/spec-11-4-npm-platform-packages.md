...
- Publishing requires `NPM_TOKEN` from repository secrets. The workflow must fail clearly when it is absent rather than half-publishing.

**Block If:**
- **Package naming is an open decision that must be settled before any code is written.** Scoped names (`@quick-studio/linux-x64`) require an npm **organization** to exist; unscoped names (`quick-studio-linux-x64`) require nothing but are unscoped-namespace squatting-adjacent. The operator has an npm account; whether the org exists is unknown. Default to **unscoped** unless the operator confirms the org, and make the name a single constant in the packaging script so switching is a one-line change.
...
|----------|--------------|---------------------------|----------------|
| Packaging | five release binaries + a version | Five platform package dirs + one main package dir, each with a generated manifest | Fails loudly on a missing binary or a missing version |
| Install on linux-x64 | `npm i -g quick-studio` | Only the linux-x64 platform package installs; the other four are skipped by `os`/`cpu` | Skips are silent and expected |
...
| Publish order | tag `v0.1.0` with `NPM_TOKEN` | Five platform packages publish, then the main package | A failed platform leg aborts before the main package publishes |
| Missing token | `NPM_TOKEN` unset | Workflow fails with a clear message, nothing published | No partial publish |
...
- Given the release binaries, when the packaging script runs, then it emits one correctly-filtered package per platform plus a dependency-free main package whose `optionalDependencies` pin exact versions.
...
- `scripts/build-npm-packages.ts` (new) — inputs: a directory of built binaries + a version string. Outputs a build tree of package directories with generated manifests. Holds the package-name constant (Block-If #1) and sets POSIX executable bits. Fails loudly on any missing input.
- `.github/workflows/publish.yml` (new) — on `v*` tags: obtain the binaries (download the 11.2 release artifacts, or `needs:` the release workflow — step-02's call, and the two workflows must not race to build the same binaries twice), run the packaging script, then `npm publish` each platform package followed by the main package, authenticated via `NPM_TOKEN`. Consider `--provenance` (requires `id-token: write` permission) — optional, flag if it complicates the token setup.
...
- [ ] Settle scoped vs unscoped package naming (Block-If #1); encode it as one constant.
...
- [ ] Write `.github/workflows/publish.yml` with the strict publish order and `NPM_TOKEN` handling.
